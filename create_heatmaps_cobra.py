...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
    ...
    with torch.inference_mode():
        ...
        # from DIAG / pathology-hhg-clam
        elif isinstance(model, (AttentionSingleBranch,AttentionMultiBranch)):
			#features = features.unsqueeze(0) #RuntimeError: tensor.H is only supported on matrices (2-D tensors). Got 3-D tensor. For batches of matrices, consider using tensor.mH
            logits, att_raw, results_dict = model(features) 
            Y_hat = torch.topk(logits, 1, dim = 1)[1].item() 
            Y_prob = F.softmax(logits, dim = 1)
			#A = results_dict["patch_logits"][...,Y_hat].view(-1, 1).cpu().numpy()
            patch_logits = results_dict["patch_logits"]
            patch_logits[...] = patch_logits[...] * patch_logits[...].shape[0]  
            attention_scores_unreshaped = F.softmax(torch.tensor(patch_logits[...]), dim=1).cpu().numpy()[...,Y_hat]  # 1 for tumor heatmap
            A = attention_scores_unreshaped.reshape(attention_scores_unreshaped.shape[0], 1)
...
def plot_patch_score_histogram(patch_scores,savepth):
    ...
    percentiles = np.percentile(patch_scores, np.linspace(0, 100, 101))
    plt.hist(patch_scores, bins=percentiles, edgecolor='black', alpha=0.75)
    plt.xlabel('Patch Score')
    plt.ylabel('Frequency')
    plt.title('Histogram of Patch Scores (100 Bins)')
    plt.show()
    plt.savefig(savepth)
    plt.close()


# adjust settings to the ones in Z:\ivan\cloned_tools\CLAM\vis_utils\heatmap_utils.py 
if __name__ == '__main__':
    ...
    print(data_args.preset)
    preset = data_args.preset
    feature_extension = data_args.feat_ext
    ...
    def_patch_params = {'white_thresh': 5, 'black_thresh': 40, 'use_padding': True, 'contour_fn': 'four_pt'}
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
            ...
            if model_args.model_type in ['addmil', 'abmil']:
                heatmap, scores, (fig,ax,cbar)= drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, 
                    alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=heatmap_args.vis_level, 
                    blank_canvas=False,	thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=True,return_scores = True, colorbar = heatmap_args.colorbar)
                if heatmap_args.colorbar:
                    fig.savefig(os.path.join(r_slide_save_dir, '{}_blockmap.png'.format(slide_id)))
                else:
                    plot_patch_score_histogram(scores, os.path.join(r_slide_save_dir, '{}_scores.png'.format(slide_id)))
            else: #CLAM original handling
                heatmap, (fig,ax,cbar) = drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, 
                    alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=heatmap_args.vis_level, 
                    blank_canvas=False, thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=True,colorbar = heatmap_args.colorbar)
                if heatmap_args.colorbar:
                    fig.savefig(os.path.join(r_slide_save_dir, '{}_blockmap.png'.format(slide_id)))
                else:
        ...
        if heatmap_args.calc_heatmap:
            if model_args.model_type == 'addmil':
        ...
        if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
            ...
            heatmap, (fig,ax,cbar) = drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object,
                                ...
                                top_left=top_left, bot_right = bot_right, colorbar = heatmap_args.colorbar)
            if heatmap_args.colorbar:
                fig.savefig(os.path.join(p_slide_save_dir, heatmap_save_name))
            elif heatmap_args.save_ext == 'jpg':
