...
from utils.distributed import merge_dicts_across_gpus, all_gather_mean
...
def ddp_setup(rank, world_size):
    ...
    master_port = get_seeded_available_port(job_id)
...
class Accuracy_Logger:
    def __init__(self, n_classes):
        ...
        self.correct = [0] * n_classes
        self.count = [0] * n_classes

    def log(self, preds, labels):
        for pred, label in zip(preds.cpu(), labels.cpu()):
            self.count[label] += 1
            self.correct[label] += int(pred == label)

    def get_summary(self, class_idx):
        if self.count[class_idx] == 0:
            return None, 0, 0
        acc = self.correct[class_idx] / self.count[class_idx]
        return acc, self.correct[class_idx], self.count[class_idx]

    def get_summary_allreduce(self, device):
        correct_tensor = torch.tensor(self.correct, device=device, dtype=torch.float32)
        count_tensor = torch.tensor(self.count, device=device, dtype=torch.float32)
        ...
        for i in range(self.n_classes):
            ...
            count = count_tensor[i].item()
            acc = correct / count if count > 0 else None
            results.append((acc, correct, count))
        return results
...
def train(datasets, cur, args,rank):
    """   
        train for a single fold
    """
    ...
    if rank == 0:
        print('\nInit Loaders...', end=' ')
    train_loader = get_split_loader(train_split, training=True, testing = args.testing, weighted = args.weighted_sample,distributed=args.distributed,rank=rank,world_size=args.world_size)
    val_loader = get_split_loader(val_split,  testing = args.testing,distributed=args.distributed,rank=rank,world_size=args.world_size)
    test_loader = get_split_loader(test_split, testing = args.testing,distributed=args.distributed,rank=rank,world_size=args.world_size)
    ...
    for epoch in range(resume_epoch,args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            resultdict = train_loop_clam(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn,args)
            ...
            stop = validate_clam(cur, epoch, model, val_loader, args.n_classes, 
                _early_stopping, writer, loss_fn, args.results_dir,args, optimizer)
        elif args.model_type in ['addmil']:   
            resultdict = train_loop_clam_addmil(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn,args)
            stop = validate_clam_addmil(cur, epoch, model, val_loader, args.n_classes, 
                _early_stopping, writer, loss_fn, args.results_dir,args, optimizer)
        else:
            resultdict = train_loop(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn,args)
        ...
        stop_tensor = torch.tensor([int(_early_stopping.early_stop)], device=args.device)
        dist.broadcast(stop_tensor, src=0)
        dist.barrier()
        if stop or stop_tensor:
            break
    ...
    results = acc_logger.get_summary_allreduce(args.device)
    if rank == 0:
        for i in range(args.n_classes):
            acc, correct, count = results[i]
            print('class {}: acc {}, correct {}/{}'.format(i, acc, correct, count))
...
def post_training(args,model,val_loader, test_loader):
    ...
    if args.model_type in ['addmil']:
        summary_call = summary_clam_addmil
    else:
        summary_call = summary
    results_val_dict, val_error, val_auc, _= summary_clam_addmil(model, val_loader, args.n_classes,args) #Changed to results_val_dict to reflect val results
    
    # Reduce averaged results across GPUs for train metrics
    # Merge result dicts
    # All-gather test/val stats
    results_val_dict = merge_dicts_across_gpus(results_val_dict, args.device)
    val_auc = all_gather_mean(torch.tensor(val_auc, device=args.device)).item()
    val_error = all_gather_mean(torch.tensor(val_error, device=args.device)).item()
    if args.rank ==0:
        print('Val error: {:.4f}, ROC AUC: {:.4f}'.format(val_error, val_auc))
    dist.barrier()

    results_test_dict, test_error, test_auc, acc_logger = summary_clam_addmil(model, test_loader, args.n_classes,args) #Changed to results_test_dict to reflect test results
    # Reduce averaged results across GPUs for train metrics
    # Merge result dicts
    # All-gather test/val stats
    acc_logger = acc_logger.get_summary_allreduce(args.device)
    results_test_dict = merge_dicts_across_gpus(results_test_dict, args.device)
    test_auc = all_gather_mean(torch.tensor(test_auc, device=args.device)).item()
    test_error = all_gather_mean(torch.tensor(test_error, device=args.device)).item()
    if args.rank ==0:
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None,args=None):
    ...
    print('\n')
    n = len(loader)
    pbar = tqdm(enumerate(loader), total = len(loader),ncols=200,desc=f"gpu{args.rank} epoch {epoch}")
...
def train_loop_clam_addmil(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None,args=None):
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None,args=None):   
    model.train()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    train_loss = 0.
    train_error = 0.
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None,args=None,optimizer = None):
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
...
def validate_clam(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None,args=None,optimizer = None):
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
    val_loss = 0.
...
def validate_clam_addmil(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None,args=None,optimizer = None):
    #device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    val_loss = 0.
...
def summary(model, loader, n_classes,args):
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    model.eval()
    test_loss = 0.
    test_error = 0.
...
def summary_clam_addmil(model, loader, n_classes,args):
    #device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    model.eval()
    test_loss = 0.
